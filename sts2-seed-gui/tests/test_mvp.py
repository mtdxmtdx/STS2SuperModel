...
class GuiMvpTests(unittest.TestCase):
    ...
    def test_cli_map_decision_and_export(self) -> None:
        ...
        try:
            session = app.create_session({"context": {"run_seed": "demo", "character": "IRONCLAD"}, "cli_path": str(ROOT / "tests" / "fake_cli.py"), "start_cli": True, "source": {"type": "manual_annotation", "id": "batch-1"}})
            ...
            value = app.add_decision(session["session_id"], {"decision_type": "route", "act": 1, "floor": 1, "selected_action": "map:1:0", "legal_actions": [{"action_id": "map:1:0"}], "sl_status": "verified_no_sl"})
            self.assertEqual(value["record"]["label_quality"], "ObservedPartial")
            self.assertEqual(value["record"]["branch_id"], "main")
            self.assertEqual(value["record"]["source_type"], "manual_annotation")
