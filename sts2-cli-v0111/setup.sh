...
# Keep the game release metadata beside the DLL so the headless process can
# reject an untested game build before generating training data.
if [ -f "$GAME_DIR/release_info.json" ]; then
    cp "$GAME_DIR/release_info.json" "lib/release_info.json"
    echo "  ✓ release_info.json"
else
    echo "  ⚠ release_info.json not found; runtime compatibility gate will reject the build"
fi
...
# Back up the unmodified game assembly before IL patching. The runtime version
# gate hashes this file so training data cannot silently use another build.
if [ -f "lib/sts2.dll" ] && [ ! -f "lib/sts2.dll.original" ]; then
    cp "lib/sts2.dll" "lib/sts2.dll.original"
    echo "  ✓ Backed up unmodified sts2.dll.original"
fi
