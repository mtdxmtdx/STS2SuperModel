    ...
    def skip_neow(self, state):
        ...
        for _ in range(20):
            ...
            if dec == "event_choice":
                ...
                # v0.111 Neow can offer card-reward relics first. Prefer a
                # direct non-reward option so map/combat tests do not depend
                # on asynchronous reward-selection timing.
                preferred = next((o for o in opts if o.get("title") == "Fishing Rod"), opts[0])
                state = self.act("choose_option", option_index=preferred["index"])
