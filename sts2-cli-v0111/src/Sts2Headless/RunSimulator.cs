...
public class RunSimulator
{
    ...
    /// <summary>
    /// Training-only snapshot endpoint. The public view is the same decision
    ...
    public Dictionary<string, object?> GetCombatSnapshot(string view)
    {
        try
        {
            var publicObservation = DetectDecisionPoint();
            var normalizedView = string.Equals(view, "teacher", StringComparison.OrdinalIgnoreCase)
                ? "teacher"
                : "public";
            var result = new Dictionary<string, object?>
            {
                ...
                ["observation_view"] = normalizedView,
                ["public_observation"] = publicObservation,
                ["public_state_hash"] = StableHash(publicObservation),
            };

            if (normalizedView == "teacher")
                result["teacher_snapshot"] = CaptureTeacherSnapshot();
            return result;
        }
        catch (Exception ex)
        {
            return ErrorWithTrace("get_combat_snapshot failed", ex);
        }
    }

    public object? GetTraceRngCounters()
    {
        try
        {
            var snapshot = CaptureTeacherSnapshot();
            return snapshot.TryGetValue("rng_counters", out var counters) ? counters : null;
        }
        catch { return null; }
    }
    ...
    private Dictionary<string, object?> CaptureTeacherSnapshot()
    {
        ...
        if (player is null || pcs is null || combat is null)
        {
            return new Dictionary<string, object?>
            {
                ["available"] = false,
                ["reason"] = "not_in_combat",
                ["rng_raw_words_exposed"] = false,
            };
        }

        Dictionary<string, object?> CardIdentity(CardModel card) => new()
        {
        ...
        try
        {
            var rng = GetRuntimeRng(player);
            if (rng is not null)
            {
                if (rng is RunRngSet typedRng)
                {
                    AddRngCounter(rngCounters, "Shuffle", typedRng.Shuffle);
                    AddRngCounter(rngCounters, "CombatCardGeneration", typedRng.CombatCardGeneration);
                    AddRngCounter(rngCounters, "CombatPotionGeneration", typedRng.CombatPotionGeneration);
                    AddRngCounter(rngCounters, "CombatCardSelection", typedRng.CombatCardSelection);
                    AddRngCounter(rngCounters, "CombatEnergyCosts", typedRng.CombatEnergyCosts);
                    AddRngCounter(rngCounters, "CombatTargets", typedRng.CombatTargets);
                    AddRngCounter(rngCounters, "CombatOrbGeneration", typedRng.CombatOrbGeneration);
                }
                CollectCounterValues(rng, "rng", rngCounters, 0);
                try
                {
                    var serialized = rng.GetType().GetMethod("ToSerializable", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.Invoke(rng, null);
                    CollectCounterValues(serialized, "rng_serialized", rngCounters, 0);
                }
                catch { }
            }
        }
        ...
        return new Dictionary<string, object?>
        {
            ["available"] = true,
            ["rng_raw_words_exposed"] = false,
            ["rng_runtime_type"] = GetRuntimeRng(player)?.GetType().FullName,
            ["rng_runtime_members"] = GetRuntimeRng(player)?.GetType().GetMembers(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Select(static member => member.Name).Distinct().OrderBy(static name => name, StringComparer.Ordinal).ToList(),
            ["round"] = combat.RoundNumber,
            ["hand"] = pcs.Hand.Cards.Select(CardIdentity).ToList(),
            ["draw_pile"] = pcs.DrawPile.Cards.Select(CardIdentity).ToList(),
            ["discard_pile"] = pcs.DiscardPile.Cards.Select(CardIdentity).ToList(),
            ["exhaust_pile"] = pcs.ExhaustPile.Cards.Select(CardIdentity).ToList(),
            ...
            ["enemy_powers"] = combat.Enemies.Where(e => e.IsAlive).Select(e => new Dictionary<string, object?>
            {
                ["enemy_id"] = StableEnemyId(e),
                ["enemy_model_id"] = e.Monster?.Id.Entry,
                ["powers"] = e.Powers.Select(power => StructuredPower(power, StableEnemyId(e))).ToList(),
            }).ToList(),
            ["rng_counters"] = rngCounters,
            ["rng_counters_available"] = rngCounters.Count > 0,
            ["rng_raw_words_exposed"] = false,
        };
    }

    private object? GetRuntimeRng(Player player)
    {
        try
        {
            var run = _runState ?? player.RunState;
            if (run is null) return null;
            return run.GetType().GetProperty("Rng", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(run)
                ?? run.GetType().GetField("_rng", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(run);
        }
        catch { return null; }
    }

    private static object? TryReadRngCounter(object? rng)
    {
        if (rng is null) return null;
        try
        {
            var direct = rng.GetType().GetProperty("Counter", BindingFlags.Public | BindingFlags.Instance)?.GetValue(rng);
            if (direct is not null) return direct;
            var method = rng.GetType().GetMethod("ToSerializable", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            var serializable = method?.Invoke(rng, null);
            if (serializable is null) return null;
            return serializable.GetType().GetProperty("counter", BindingFlags.Public | BindingFlags.Instance)?.GetValue(serializable)
                ?? serializable.GetType().GetProperty("Counter", BindingFlags.Public | BindingFlags.Instance)?.GetValue(serializable);
        }
        catch { return null; }
    }

    private static void CollectCounterValues(object? value, string path, Dictionary<string, object?> output, int depth)
    {
        if (value is null || depth > 5 || value is string || value.GetType().IsPrimitive) return;
        try
        {
            var counter = value.GetType().GetProperty("Counter", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(value)
                ?? value.GetType().GetProperty("counter", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(value);
            if (counter is not null) output[path] = counter;
        }
        catch { }

        foreach (var property in value.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
        {
            if (property.GetIndexParameters().Length > 0 || property.Name.Contains("State", StringComparison.OrdinalIgnoreCase)) continue;
            try
            {
                var nested = property.GetValue(value);
                if (nested is not null && nested is not string && !nested.GetType().IsPrimitive)
                    CollectCounterValues(nested, $"{path}.{property.Name}", output, depth + 1);
            }
            catch { }
        }
    }

    private static void AddRngCounter(Dictionary<string, object?> output, string name, Rng source)
    {
        try
        {
            output[name] = source.ToSerializable().counter;
        }
        catch
        {
    ...
    public string NormalizeTraceAction(JsonElement command)
    {
        var cmd = command.TryGetProperty("cmd", out var cmdElement) ? cmdElement.GetString() ?? string.Empty : string.Empty;
        if (cmd != "action") return cmd;
        var action = command.TryGetProperty("action", out var actionElement) ? actionElement.GetString() ?? string.Empty : string.Empty;
        ...
        try
        {
            var player = _runState?.Players.FirstOrDefault();
            var pcs = player?.PlayerCombatState;
            if (action == "play_card" && args.TryGetProperty("card_index", out var cardIndexElement) && pcs is not null)
            {
                var cardIndex = cardIndexElement.GetInt32();
                if (cardIndex >= 0 && cardIndex < pcs.Hand.Cards.Count)
                {
                    var targetId = "none";
                    if (pcs.Hand.Cards[cardIndex].TargetType == TargetType.AnyEnemy &&
                        args.TryGetProperty("target_index", out var targetIndexElement))
                    {
                        var targets = CombatManager.Instance.DebugOnlyGetState()?.Enemies.Where(e => e.IsAlive).ToList();
                        var targetIndex = targetIndexElement.GetInt32();
                        if (targets is not null && targetIndex >= 0 && targetIndex < targets.Count)
                            targetId = StableEnemyId(targets[targetIndex]);
                    }
                    return $"play_card:{StableCardId(pcs.Hand.Cards[cardIndex])}:{targetId}";
                }
            }
            if (action == "use_potion" && args.TryGetProperty("potion_index", out var potionIndexElement) && player is not null)
            {
                var slot = potionIndexElement.GetInt32();
                var potion = player.GetPotionAtSlotIndex(slot);
                if (potion is not null) return $"use_potion:{StablePotionId(potion)}";
            }
            if (action == "select_cards" && _cardSelector.HasPending && _cardSelector.PendingOptions is not null &&
                args.TryGetProperty("indices", out var indicesElement))
            {
                var indices = (indicesElement.GetString() ?? indicesElement.ToString())
                    .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                    .Select(value => int.TryParse(value, out var index) ? index : -1)
                    .Where(index => index >= 0 && index < _cardSelector.PendingOptions.Count)
                    .ToArray();
                var choiceId = StableCardChoiceId(
                    _cardSelector.PendingOptions,
                    _cardSelector.PendingMinSelect,
                    _cardSelector.PendingMaxSelect);
                var selectedIds = indices.Select(index => StableCardId(_cardSelector.PendingOptions[index]));
                return $"select_cards:{choiceId}:{string.Join(',', selectedIds)}";
            }
        }
        catch { }
        var fields = args.EnumerateObject().OrderBy(static p => p.Name, StringComparer.Ordinal).Select(static p => $"{p.Name}={p.Value}");
        return $"{action}:{string.Join(',', fields)}";
    }

    private string StableCardId(CardModel card)
    {
        if (_stableCardIds.TryGetValue(card, out var existing)) return existing;
        var runtime = TryRuntimeIdentity(card);
        var baseId = card.Id.Entry + (card.IsUpgraded ? "+" : string.Empty);
        var value = runtime is { Length: > 0 }
            ? $"card-runtime:{runtime}"
            : $"card:{baseId}:{NextOrdinal(_cardOrdinals, baseId):D3}";
        _stableCardIds[card] = value;
        return value;
    }

    private string StablePotionId(PotionModel potion)
    {
        if (_stablePotionIds.TryGetValue(potion, out var existing)) return existing;
        var runtime = TryRuntimeIdentity(potion);
        var baseId = potion.Id.Entry;
        var value = runtime is { Length: > 0 }
            ? $"potion-runtime:{runtime}"
            : $"potion:{baseId}:{NextOrdinal(_potionOrdinals, baseId):D3}";
        _stablePotionIds[potion] = value;
        return value;
    }

    private static int NextOrdinal(Dictionary<string, int> ordinals, string key)
    {
        var value = ordinals.TryGetValue(key, out var current) ? current : 0;
    ...
    {
        foreach (var name in new[] { "InstanceId", "InstanceID", "Uuid", "UUID", "Guid", "UniqueId", "UniqueID" })
        {
            try
            {
    ...
    private static object? SafeRelicCounter(RelicModel relic)
    {
        try { return relic.ShowCounter ? relic.DisplayAmount : null; }
        catch { return null; }
    }

    private string StableCardChoiceId(IEnumerable<CardModel> options, int minSelect, int maxSelect)
    {
        var identity = new
        {
            kind = "card_select",
            ...
            option_ids = options.Select(StableCardId).ToArray(),
        };
        return $"choice:{StableHash(identity)[..24]}";
    }

    private List<Dictionary<string, object?>> BuildCardSelectionCandidates(
        ...
        out bool complete)
    {
        ...
        void AddSelection()
        {
            var selectedIds = selected.Select(index => StableCardId(options[index])).ToArray();
            result.Add(new Dictionary<string, object?>
            {
                ["kind"] = "Choice",
                ["action_id"] = $"choice:{choiceId}:{string.Join(',', selectedIds)}",
                ["choice_id"] = choiceId,
                ["selected_card_instance_ids"] = selectedIds,
                ["effective_energy_cost"] = 0,
                ["legal"] = true,
            });
        }

        void Enumerate(int start, int remaining)
        {
            if (!isComplete) return;
            if (remaining == 0)
            {
                AddSelection();
                if (result.Count >= limit) isComplete = false;
                return;
            }
            for (var index = start; index <= options.Count - remaining && isComplete; index++)
            {
                selected.Add(index);
        ...
        var minimum = Math.Clamp(minSelect, 0, options.Count);
        var maximum = Math.Clamp(maxSelect, minimum, options.Count);
        for (var size = minimum; size <= maximum && isComplete; size++)
            Enumerate(0, size);
        complete = isComplete;
        return result;
    }

    private Dictionary<string, object?> StructuredPower(PowerModel power, string ownerId)
    {
        var dynamicVars = new Dictionary<string, object?>(StringComparer.Ordinal);
        try
        {
            foreach (var variable in power.DynamicVars.Values)
                dynamicVars[variable.Name] = variable.BaseValue;
        }
        catch { }

        var counters = new Dictionary<string, object?>(StringComparer.Ordinal);
        foreach (var property in power.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
        {
            if (property.GetIndexParameters().Length > 0 ||
                !(property.Name.Contains("Counter", StringComparison.OrdinalIgnoreCase) ||
                  property.Name.Contains("Charge", StringComparison.OrdinalIgnoreCase) ||
                  property.Name.Contains("Remaining", StringComparison.OrdinalIgnoreCase) ||
                  property.Name.Contains("Times", StringComparison.OrdinalIgnoreCase) ||
                  property.Name.Contains("Uses", StringComparison.OrdinalIgnoreCase)))
                continue;
            try
            {
                var value = property.GetValue(power);
                if (value is byte or sbyte or short or ushort or int or uint or long or ulong or float or double or decimal)
                    counters[property.Name] = value;
            }
            catch { }
        }

        var methodNames = power.GetType()
            .GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly)
            .Select(static method => method.Name)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var phases = new SortedSet<string>(StringComparer.Ordinal);
        if (methodNames.Any(name => name.Contains("Play", StringComparison.OrdinalIgnoreCase) || name.Contains("Card", StringComparison.OrdinalIgnoreCase))) phases.Add("CardPlay");
        if (methodNames.Any(name => name.Contains("TurnStart", StringComparison.OrdinalIgnoreCase) || name.Contains("SideTurnStart", StringComparison.OrdinalIgnoreCase))) phases.Add("TurnStart");
        if (methodNames.Any(name => name.Contains("TurnEnd", StringComparison.OrdinalIgnoreCase) || name.Contains("SideTurnEnd", StringComparison.OrdinalIgnoreCase))) phases.Add("TurnEnd");
        if (methodNames.Any(name => name.Contains("Damage", StringComparison.OrdinalIgnoreCase) || name.Contains("Hit", StringComparison.OrdinalIgnoreCase))) phases.Add("Damage");
        if (methodNames.Any(name => name.Contains("Block", StringComparison.OrdinalIgnoreCase))) phases.Add("Block");
        if (methodNames.Any(name => name.Contains("Draw", StringComparison.OrdinalIgnoreCase))) phases.Add("Draw");
        if (methodNames.Any(name => name.Contains("Death", StringComparison.OrdinalIgnoreCase) || name.Contains("Fatal", StringComparison.OrdinalIgnoreCase))) phases.Add("Death");
        if (phases.Count == 0) phases.Add("Passive");

        string? applierId = null;
        foreach (var propertyName in new[] { "Applier", "ApplierPlayer", "Source" })
        {
            try
            {
                var applier = power.GetType().GetProperty(propertyName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(power);
                if (applier is Creature creature)
                    applierId = creature.Monster is null ? "player" : StableEnemyId(creature);
                else if (applier is Player)
                    applierId = "player";
                if (applierId is not null) break;
            }
            catch { }
        }
        ...
        return new Dictionary<string, object?>
        {
            ...
            ["description"] = _loc.Bilingual("powers", power.Id.Entry + ".description"),
            ["runtime_type"] = power.GetType().FullName,
            ["owner_id"] = ownerId,
            ["applier_id"] = applierId,
            ["amount"] = power.Amount,
            ["dynamic_vars"] = dynamicVars,
            ["internal_counters"] = counters,
            ["trigger_phases"] = phases.ToList(),
            ["source"] = "game_runtime",
            ["support"] = "state_captured",
            ["evidence"] = "LiveObserved",
            ["trigger_phase_evidence"] = "HeuristicInferred",
            ["source_version"] = "v0.111.0",
        };
    }
    ...
    private Dictionary<string, object?> DetectDecisionPoint()
    {
        ...
        if (_cardSelector.HasPending && _cardSelector.PendingOptions != null)
        {
            ...
            var choiceId = StableCardChoiceId(
                ...
                _cardSelector.PendingMinSelect,
                _cardSelector.PendingMaxSelect);
            var actionCandidates = BuildCardSelectionCandidates(
                _cardSelector.PendingOptions,
                _cardSelector.PendingMinSelect,
                _cardSelector.PendingMaxSelect,
                choiceId,
                out var actionCandidatesComplete);
            return new Dictionary<string, object?>
            {
                ["type"] = "decision",
                ...
                ["action_candidates_complete"] = actionCandidatesComplete,
                ["choice_restriction"] = actionCandidatesComplete ? null : "choice_space_exceeds_export_limit",
                ["player"] = PlayerSummary(player),
            };
        }
    ...
    private Dictionary<string, object?> CombatPlayState(Player player)
    {
        ...
        // Player powers/buffs
        var playerPowers = player.Creature?.Powers?.Select(pw => StructuredPower(pw, "player")).ToList();
    ...
    private List<Dictionary<string, object?>> BuildActionCandidates(
        List<Dictionary<string, object?>> hand,
        ...
        Player player)
    {
        ...
        {
            var instanceId = card.GetValueOrDefault("instance_id")?.ToString();
            if (string.IsNullOrWhiteSpace(instanceId)) continue;
            var modelId = card.GetValueOrDefault("id")?.ToString();
            var legal = card.GetValueOrDefault("can_play") is bool canPlay && canPlay;
            var cost = card.GetValueOrDefault("cost") is int intCost ? intCost : Convert.ToInt32(card.GetValueOrDefault("cost") ?? 0);
            var targetType = card.GetValueOrDefault("target_type")?.ToString();
            if (targetType == "AnyEnemy")
            {
                foreach (var enemy in enemies)
                {
                    var targetId = enemy.GetValueOrDefault("instance_id")?.ToString() ?? "unknown";
                    result.Add(new Dictionary<string, object?>
                    {
                        ["kind"] = "PlayCard", ["action_id"] = $"play:{instanceId}:{targetId}",
                        ["source_model_id"] = modelId, ["source_instance_id"] = instanceId,
                        ["target_id"] = targetId, ["effective_energy_cost"] = cost,
                        ["legal"] = legal,
                    });
                }
            }
            else
            {
                result.Add(new Dictionary<string, object?>
                {
                    ["kind"] = "PlayCard", ["action_id"] = $"play:{instanceId}:none",
                    ["source_model_id"] = modelId, ["source_instance_id"] = instanceId,
                    ["effective_energy_cost"] = cost, ["legal"] = legal,
                });
            }
        }
        foreach (var potion in player.Potions ?? [])
        {
            ...
            var instanceId = StablePotionId(potion);
            result.Add(new Dictionary<string, object?>
            {
                ["kind"] = "UsePotion", ["action_id"] = $"potion:{instanceId}",
                ["source_model_id"] = potion.Id.Entry, ["source_instance_id"] = instanceId,
                ["effective_energy_cost"] = 0, ["legal"] = true,
            });
        }
        result.Add(new Dictionary<string, object?>
        {
    ...
    private static void EnsureModelDbInitialized()
    {
        ...
        // v0.111 gates ReflectionHelper.ModTypes behind the normal mod-loader
        // lifecycle. Headless runs have no mod loader, so initialize the two
        // backing registries directly: all game types and an empty mod set.
        try
        {
            var reflectionHelper = typeof(MegaCrit.Sts2.Core.Helpers.ReflectionHelper);
            var allTypes = reflectionHelper.GetField("_allTypes",
                BindingFlags.Static | BindingFlags.NonPublic);
            var modTypes = reflectionHelper.GetField("_modTypes",
                BindingFlags.Static | BindingFlags.NonPublic);
            allTypes?.SetValue(null, Array.Empty<Type>());
            modTypes?.SetValue(null, Array.Empty<Type>());
            var getter = reflectionHelper.GetProperty("ModTypes",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod;
            var availabilityGetter = reflectionHelper.GetProperty("SubtypesAvailable",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod;
            var prefix = typeof(RunSimulator).GetMethod(nameof(ReflectionModTypesPrefix),
                BindingFlags.Static | BindingFlags.NonPublic);
            var availabilityPrefix = typeof(RunSimulator).GetMethod(nameof(ReflectionSubtypesAvailablePrefix),
                BindingFlags.Static | BindingFlags.NonPublic);
            var harmony = new Harmony("sts2headless.reflectionhelper");
            if (getter != null && prefix != null)
            {
                harmony.Patch(getter, prefix: new HarmonyMethod(prefix));
                Console.Error.WriteLine("[INFO] Patched ReflectionHelper.ModTypes");
            }
            if (availabilityGetter != null && availabilityPrefix != null)
            {
                harmony.Patch(availabilityGetter, prefix: new HarmonyMethod(availabilityPrefix));
                Console.Error.WriteLine("[INFO] Patched ReflectionHelper.SubtypesAvailable");
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[WARN] ReflectionHelper subtype gate: {ex.Message}");
        }
        ...
        // ModelDb.Inject may rebuild ReflectionHelper caches; restore the
        // empty mod-type cache after registration as well.
        try
        {
            var reflectionHelper = typeof(MegaCrit.Sts2.Core.Helpers.ReflectionHelper);
            reflectionHelper.GetField("_allTypes", BindingFlags.Static | BindingFlags.NonPublic)
                ?.SetValue(null, subtypes.ToArray());
            reflectionHelper.GetField("_modTypes", BindingFlags.Static | BindingFlags.NonPublic)
                ?.SetValue(null, Array.Empty<Type>());
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[WARN] ReflectionHelper mod cache: {ex.Message}");
        }
    ...
    private static bool ReflectionModTypesPrefix(ref Type[] __result)
    {
        __result = Array.Empty<Type>();
        return false;
    }

    private static bool ReflectionSubtypesAvailablePrefix(ref bool __result)
    {
        __result = true;
        return false;
    }
